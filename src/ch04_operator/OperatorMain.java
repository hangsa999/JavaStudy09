...
public class OperatorMain {
    ...
    public static void main(String[] args) {
        ...
        // 게시판 페이징 구현
        int totalCountGul = 3122;  // 게시판에 글이 총 3122개 있다.
        int showCountPage = 15;  // 한 페이지에서 보여줄 글의 수가 15개
        ...
        // != 는 왼쪽과 오른쪽이 다른지 비교
        // 다르다면 true, 같다면 false (== 와 반대)
        System.out.println(numOne != numFive);
        System.out.println(numOne != 1);

        // 문자(char)도 비교연산자를 통해 이름순 정렬에 사용할 수 있다.
        System.out.println('가' < '나');  // 유니코드당 '가'가 '나'보다 먼저 위치(=작은 숫자)
        ...
        // inputId 의 글자수가 7글자 이상이면 통과, 7글자 미만이면 탈락
        ...
        // score = 85 일 때
        // score가 90점 이상이면 "A", 90점 미만이면 "B"가 grade에 담기도록 코드 작성
        int score = 75;
        ...
        // 10 <= x <= 11
        // 10 <= x && x <= 11
        System.out.println(10 <= inputPhone.length() && inputPhone.length() <= 10);

        // !
        // !를 boolean 타입 앞에 붙여주면 (true/false) 값을 뒤집는다.
        // 이름은 1글자 이상 = 이름은 empty가 아니어야 한다.
        // name이 empty 일떄 true 리턴
        // !inputName.isEmpty() = name이 empty가 아니라면 true

        System.out.println(!inputName.isEmpty() && inputAge >= 14 && (inputPhone.length() == 10 || inputPhone.length() == 11));
        ...
        // 비트 연산자 XOR
        // ^
        // 각 자리수를 비교해서 서로 다른 경우에만 그 자리에 1이 남음산
