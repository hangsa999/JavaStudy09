...
public class ReceiveThread extends Thread{
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                // inputStream 안에 있는 내용 읽기 (SendThread 에서 상대방이 writer.flush() 시 실행)
                String inputMag = reader.readLine();  // SendThread에서 상대방에 writer.flush() 할 때까지 대기
                // 넘어온 메시지 출력
                System.out.println(inputMag);

                // 넘어온 메시지가 exit 이면 ReciveThread 중지
                if ("exit".equals(inputMag)) {
                    break;
                }
            }
